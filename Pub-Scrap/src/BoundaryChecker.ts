...
export class BoundaryChecker {
  private static async getCoordinatesData(target: string) {
    try {
      const osmResponse = await fetch(
        `https://nominatim.openstreetmap.org/search.php?q=${target}&polygon_geojson=1&format=jsonv2`,
      );
      ...
      let idx = 0;
      if (osmData.length > 1) {
        osmData.forEach((singleItem, index) =>
          console.log(`${index} - ${singleItem.display_name}`),
        );
        ...
        const enteredValue = await new Promise<string>((resolve) => {
  ...
  private static createCoordinatesFromBoundaryValues(boundingBox) {
  ...
  private static async getBoundaryData(coordinatesdata, data) {
    const finalData = [];
    let count: number = 0;
    let polygonFigure;
